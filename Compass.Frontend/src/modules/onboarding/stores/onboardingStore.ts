...
import { useProgressStore } from '@/modules/analytics/stores/progressStore';
...
// 1. Injeção de Projetos e Metas (Normalizada)
    const p1 = { id: 'proj-core-101', name: 'compass-core', description: 'Migração do Motor Tático', lastUsedAtUtc: new Date().toISOString(), goalId: null };
    
    //  FIX 1: Adicionado goalId: null no p2 para satisfazer a interface
    const p2 = { id: 'proj-arch-202', name: 'local-first-book', description: 'Arquiteturas resilientes', lastUsedAtUtc: new Date(Date.now() - 86400000 * 2).toISOString(), goalId: null };
    
    // 🛡️ ARQ: Injeção Segura na Fonte de Verdade
    ...
    //  FIX 2: A linha projectsStore.isServingFromCache = true; foi deletada pois removemos isso do projectsStore na Etapa 3!
