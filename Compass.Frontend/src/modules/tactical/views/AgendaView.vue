...
import { SchedulesApi } from '@/modules/tactical/api/schedules.api';
import { useTimeResize } from '@/modules/tactical/composables/useTimeResize';
import { isQuickCaptureOpen } from '@/shared/composables/useKeyboardShortcuts';
import { useToastStore } from '@/shared/stores/toastStore'; //  IMPORT DO TOAST
import type { HorizonOption } from '@/modules/tactical/components/TacticalHorizonBar.vue';
...
onMounted(async () => {
  ...
  // ARQ: Busca apenas o contexto efêmero da tela (Schedule e NowEngine).
  // Os compromissos já foram hidratados globalmente pelo App.vue!
  const [scheduleData] = await Promise.all([
    SchedulesApi.fetchTodaySchedule().catch(() => baseSchedule.value),
