...
// Computada reativa para trocar a lista desenhada de acordo com a aba selecionada!
const displayedItems = computed(() => {
  if (journalStore.activeTab === 'week') {
    // Retorna os dados da semana se existirem no store, com fallback seguro para os de hoje
...
onMounted(async () => {
  ...
  // Se a sua store de auditoria tiver um método de carga inicial, chamamos aqui:
  if (typeof (journalStore as any).fetchAuditLogs === 'function') {
    await (journalStore as any).fetchAuditLogs();
  }
});
</script>
...
<template>
  <div class="max-w-4xl mx-auto space-y-8 select-none pb-12">
    ...
    <!-- LISTA CRONOLÓGICA DE AUDITORIA (Conectada em displayedItems!) -->
